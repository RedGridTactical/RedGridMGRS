 ...
 *
 * Zero dependencies — wordlist bundled as JSON.
 */
...
function encode(lat, lon) {
  ...
  const g0 = Number(latStr.substring(0, 4));
  const g1 = Number(lonStr.substring(0, 4)) + 2000;
  const g2 = Number(latStr.substring(4, 6) + lonStr.substring(4, 5)) + 5610;
  const g3 = Number(latStr.substring(6, 7) + lonStr.substring(5, 7)) + 6610;
...
/**
 * Format lat/lon as a FixPhrase display string.
 * @param {number} lat
 * @param {number} lon
 * @returns {string}
 */
